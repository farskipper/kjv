...
for(i=0; i<lines.length; i++){
    ...
    if(!started_first_book){
        if(_.has(books, line)){
            started_first_book = true;
        }else{
            out.push(["TXT", line]);
            continue;
        }
    ...
    var m_chap = /^(CHAPTER|PSALM) ([0-9]+)$/.exec(line);
    var m_verse = /^([0-9]+) (.*)$/.exec(line);
    if(_.has(books, line)){
        out.push(["BOOK", books[line]]);
    }else if(m_chap){
        out.push(["CHAPTER", toInt(m_chap[2])]);
    }else if(m_verse){
        var text = m_verse[2].trim();
        if(text[0] === "#"){
            out.push(["PARAGRAPH"]);
        }
        out.push(["VERSE", toInt(m_verse[1]), text]);
    }else{
        out.push(["TXT", line]);
    }
}
